        ...
        <div>
            <Card style={{marginTop:'10px'}}>
                ...
                <Collapse isOpen={this.state.collapse}>
                    <CardBody>
                        <Row>
                            <Col sm style={{marginBottom:'25px'}} ><h5>Ingredients:</h5><ul className="paddingLeft10">{this.props.ingredients.map((ingredient,index)=>(
                                <li key={index}>{ingredient}</li>
                            ))}</ul></Col>
                            <Col sm ><h5>Cooking Directions:</h5><ol className='paddingLeft10'>{this.props.directions.map((step,index)=>(
